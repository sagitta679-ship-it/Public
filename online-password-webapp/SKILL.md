...
4. **解锁层**：全屏固定遮罩 + 居中卡片（标题/说明/密码框/确认密码框(仅首次)/错误提示/进入按钮），现代简约黑白灰。**锁屏默认显示、工具主容器默认隐藏**（`style="display:none"`），解锁成功（hideLock）后再切换显示——否则页面加载时会闪现工具界面再跳锁屏。工具主容器必须带 id（只有 class 的话 `$('wrap')` 找不到，解锁后报错导致界面空白）
...
7. **锁屏与工具界面的显示切换**：页面加载时锁屏默认显示、工具区默认隐藏，否则会闪现工具界面再跳锁屏（用户实测 0.5 秒闪现）；工具容器必须有 id——本技能曾因只有 class 没有 id，`$('wrap')` 返回 null，解锁后 hideLock 报错中断，界面空白。改完必须全流程测：设置密码→解锁→工具界面可见
